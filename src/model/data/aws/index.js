...
//will help to deal with data in our bucket
const { PutObjectCommand, GetObjectCommand, DeleteObjectCommand } = require('@aws-sdk/client-s3');

//will help to deal with data in our DynamoDB
...
async function writeFragmentData(ownerId, id, data) {
  ...
  try {
    logger.info(`cmd - ${JSON.stringify(command)}`);
...
async function deleteFragment(ownerId, id) {
  ...
  const paramsDdb = {
    TableName: process.env.AWS_DYNAMODB_TABLE_NAME,
    // specify the key of fragment that needs to be deleted
    Key: { ownerId, id },
  };

  // Create a DELETE Object command to delete data from S3
  const BktCmd = new DeleteObjectCommand(paramsBkt);

  // Create a DELETE Object command to delete data from Ddb
  const DdbCmd = new DeleteCommand(paramsDdb);

  try {
    // sending cmd to delete a fragment from S3
    const data = await s3Client.send(BktCmd);
    // Delete metadata
    const metaData = await ddbDocClient.send(DdbCmd);

    //logging deleted data
    logger.debug(`Fragment Deleted - Info: ${metaData}, Data: ${data}`);
  } catch (err) {
    // If anything goes wrong while deleting, log enough info that we can debug
    const { Key } = paramsBkt;
    logger.error({ err, Key }, 'Error deleting fragment data from S3/Ddb');
